...
class TabManager {
    ...
    async init() {
        ...
        if (saved && saved.tabs && saved.tabs.length > 0) {
            ...
            this.createNewTab(t('nav.defaultFile'), t('content.default') + window.defaultCSS, true);
        }
    }
    ...
    renderTabs() {
        ...
        this.tabs.forEach(tab => {
            ...
            div.onclick = () => this.switchToTab(tab.id);
            ...
            div.innerHTML = `
                ${dot}
                <span class="text-xs sm:text-sm max-w-[120px] truncate ${isActive ? 'text-blue-600 dark:text-blue-400 font-semibold' : 'text-gray-600 dark:text-gray-400'}">${tab.name}</span>
...
window.onI18nReady = () => {
    ...
    tabManager.tabs.forEach(tab => {
        if (tab.isDefault) {
            tab.content = t('content.default') + window.defaultCSS;
...
window.onLanguageChanged = window.onI18nReady;
...
function switchToCode() {
    getActive().mode = 'code';
    document.getElementById('mode-code-btn').classList.replace('bg-gray-200', 'bg-blue-600');
    document.getElementById('mode-code-btn').classList.replace('dark:bg-gray-800', 'bg-blue-600');
    document.getElementById('mode-code-btn').classList.replace('text-gray-700', 'text-white');
    document.getElementById('mode-code-btn').classList.replace('dark:text-gray-300', 'text-white');
    
    document.getElementById('mode-visual-btn').classList.replace('bg-blue-600', 'bg-gray-200');
    document.getElementById('mode-visual-btn').classList.replace('text-white', 'text-gray-700');
    document.getElementById('mode-split-btn').classList.replace('bg-blue-600', 'bg-gray-200');
    document.getElementById('mode-split-btn').classList.replace('text-white', 'text-gray-700');
...
function switchToVisual() {
    getActive().mode = 'visual';
    document.getElementById('mode-visual-btn').classList.replace('bg-gray-200', 'bg-blue-600');
    document.getElementById('mode-visual-btn').classList.replace('dark:bg-gray-800', 'bg-blue-600');
    document.getElementById('mode-visual-btn').classList.replace('text-gray-700', 'text-white');
    document.getElementById('mode-visual-btn').classList.replace('dark:text-gray-300', 'text-white');
    
    document.getElementById('mode-code-btn').classList.replace('bg-blue-600', 'bg-gray-200');
    document.getElementById('mode-code-btn').classList.replace('text-white', 'text-gray-700');
    document.getElementById('mode-split-btn').classList.replace('bg-blue-600', 'bg-gray-200');
    document.getElementById('mode-split-btn').classList.replace('text-white', 'text-gray-700');
...
function switchToSplit() {
    getActive().mode = 'split';
    document.getElementById('mode-split-btn').classList.replace('bg-gray-200', 'bg-blue-600');
    document.getElementById('mode-split-btn').classList.replace('dark:bg-gray-800', 'bg-blue-600');
    document.getElementById('mode-split-btn').classList.replace('text-gray-700', 'text-white');
    document.getElementById('mode-split-btn').classList.replace('dark:text-gray-300', 'text-white');
...
function syncCodeToVisual() {
    ...
    doc.open(); doc.write(editor.getValue()); doc.close(); doc.body.contentEditable = true;
}
...
function updateUI() {
    ...
    if (!tab.isDefault) {
        ...
        else if (name.endsWith('.txt') || name.endsWith('.md') || name.endsWith('.srt') || !name.includes('.')) { mode = 'null'; lintType = false; }
...
function toggleTheme() {
    ...
    if (html.classList.contains('dark')) { html.classList.remove('dark'); icon.classList.replace('fa-sun', 'fa-moon'); editor.setOption('theme', 'default'); if(mergeView) mergeView.edit.setOption('theme','default'); } 
    else { html.classList.add('dark'); icon.classList.replace('fa-moon', 'fa-sun'); editor.setOption('theme', 'dracula'); if(mergeView) mergeView.edit.setOption('theme','dracula'); }
}
